...
=O365Interactive=，无需写入任何密钥——真正的登录是浏览器弹窗完成
的，token 由 davmail 自己缓存），然后用 =start-process= 拉起来，作为
独立后台服务持续跑，不随 Emacs 退出而结束——重新拉起一个 JVM 有启动
开销，没道理绑定 Emacs 的生命周期。因此「是否已经在跑」不能看 Emacs
自己的进程列表（那样只能看到当前会话起的进程，看不到上一次会话留下
...
*注意*：davmail 本地 IMAP/SMTP 那端到底要不要一个「像样」的密码、还
...
=%INBOX:user/clear@host:port!= 语法把连接信息整个编码进 folder 名里
（SDF、Gmail 走 TLS 加 =/clear@...!= ，Hotmail 走本地 davmail 不需要
TLS 后缀）。只在文件不存在时写一次，写完之后就是用户自己维护的文件
（加子文件夹、改名随意），不会被覆盖。
#+begin_src emacs-lisp
  (with-eval-after-load 'wl
