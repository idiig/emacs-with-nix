...
- 同时协同 emacs 插件和其依赖的外部工具 ::
  ...
  =consult-ripgrep= 会寻找可运行的 =ripgrep= 调用。这样我需要脱离 elisp 的
  设定文件去下载这个外部工具。还有很多编程语言也有这个问题。我的习惯是
  希望针对特定的项目文件夹定制所需的环境（比如使用 nix 和
  .dir-locals.el）。但是，类似于语言服务器这类本质与项目无关，但与我的
  编辑环境有关的设定，我倾向于使用与项目无关的环境。
** 其他方式做不到的
...
这里稍微多做一些解释。在配置节里面，我利用了org-babel的一些特性。首先
我们在一个入口代码块中使用了 =:noweb=yes= 。然后在后面的代码块中我们使用
=:noweb-ref= 参照入口代码块，最后后续的这些代码块都会在 =org-babel-tangle=
(=C-c C-v C-t=) 时合并到这个入口代码块中输出为 flake.nix。
* 如何使用
:PROPERTIES:
:header-args: :tangle no
:END:
