...
个 folder 属于哪个账号"的映射。命中后覆盖 ="From"= 以及
=wl-smtp-posting-server/-port/-user/-authenticate-type/-connection-type=。
从 =idiig/mail-accounts= 循环生成 3 条，不手写。

对着 =wl-draft.el= 源码确认过一个细节：条件和 value 都是 *没被求值
的* Lisp form，Wanderlust 自己在真正撰写邮件、命中某条规则时才会
=eval= 它们（官方手册的示例也是直接 =quote= 一整个 alist，不是
反引号拼值）。所以生成这张表的时候，="From"= /
=wl-smtp-posting-user= 对应的值故意保留成
...
外观只加一条：常驻显示 folder 面板 (=wl-stay-folder-window=)，3 个
账号切来切去的时候不用每次都重新展开。
#+begin_src emacs-lisp
