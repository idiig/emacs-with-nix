...
:header-args: :noweb-ref ecfg
:END:
账号相关的敏感信息（用户名、密码等）不直接写死在这份配置里，参照
[[*密钥管理 (=sops=)][=idiig/get-sops-secret-value=]] 从 SOPS 加密的
=~/.config/secrets.yaml= 中按需读取。
**** TODO 入口与启动
- 设 =mail-user-agent= 为 =wl-user-agent=，让 =C-x m= 走 Wanderlust 撰写
  邮件
- =use-package wl :commands (wl wl-draft)= 惰性加载，避免拖慢启动
**** TODO 收发服务器 (IMAP / SMTP)
- 收信：=elmo-imap4-default-server= / =-user= / =-authenticate-type= /
  =-port= / =-stream-type=
- 发信：=wl-smtp-posting-server= / =-posting-port= /
  =-authenticate-type= / =-connection-type=
- 用户名、密码通过 =idiig/get-sops-secret-value= 从 =secrets.yaml=
  读取，配置文件里不出现明文
**** TODO 本地目录与订阅 folders
- =elmo-localdir-folder-path= 指定本地缓存/归档根目录（默认
  =~/Mail=）
- =wl-folders-file=（默认 =~/.folders=）用 elmo 的前缀语法订阅文件
  夹，如 =%inbox= 表示 IMAP，=+inbox= 表示本地 MH 目录
- 是否要把 =~/.folders= 也纳入这份 Nix 配置一起管理，多机同步时再
  决定
**** TODO 签名与外观
- =wl-signature-file-name= 指定签名文件
- =wl-stay-folder-window= / =wl-folder-window-width= 控制目录树的显
  示与宽度
- =wl-summary-weekday-name-lang= 等摘要显示细节
** 文档写作
:PROPERTIES:
:header-args: :noweb-ref ecfg
:END:
