...
:header-args: :noweb-ref ecfg
:END:
下面按依赖顺序拆成几节，=davmail= 网关要放在最前面，因为「账号」那
一节的 Hotmail 条目要引用它的本地端口。
...
=~/mails/.folders= ）的内容，每个账号一行，用 elmo 的
=%INBOX:"user"/clear@host:port!= 语法把连接信息整个编码进 folder 名
里（SDF、Gmail 走 TLS 加 =/clear@...!= ，Hotmail 走本地 davmail 不需
要 TLS 后缀）。用户名一定要用双引号包起来——Gmail/Hotmail 的登录名本
身就是完整邮箱地址，也带 =@=，跟分隔 user/host 的那个 =@= 撞在一起会
解析错乱，实测（见下面 =idiig/mail-account-folder-line= 的 docstring）
不加引号直接导致 elmo 连不上；SDF 的用户名没有 =@= 所以当时看不出问
题。只在文件不存在时写一次，写完之后就是用户自己维护的文件（加子文
件夹、改名随意），不会被覆盖。
#+begin_src emacs-lisp
  (with-eval-after-load 'wl
...
回信/写信时才有值——直接 =C-x m= 撰写新邮件时它是 nil，上面 3 条规
则一条都不会命中，From/SMTP 就没人设置。参照
...
对着 =wl-draft.el= 源码确认过一个细节：条件和 value 都是 *没被求值
...
外观加两条：常驻显示 folder 面板 (=wl-stay-folder-window=)，3 个账号
...
=:config= 结尾用 =advice-add 'wl :before= 挂一个 =idiig/wl-bootstrap=，
每次打开 =wl= 之前把前面几节的 setup 函数（davmail、folders、签名、
auth-source 缓存）都跑一遍——各自都是幂等操作，重复跑无副作用。
#+begin_src emacs-lisp
