...
#+begin_src emacs-lisp :noweb yes
  (use-package puni
    ...
    <<idiig/puni-backward-kill-word-or-region>>
    <<idiig/puni-backward-hungry-delete>>
    <<idiig/puni-forward-hungry-delete>>  
...
#+begin_src emacs-lisp :noweb-ref idiig/puni-forward-hungry-delete
...
- 首先检查光标前面是否有连续的空白字符。
- 使用 =looking-back= 用于判断光标之前是否有符合正则表达式 =(+ blank)= 的字符序列。 =(line-beginning-position)= 确保不会查看超出行首的内容。
- 如果有连续的空白字符：
  - 使用 =skip-chars-backward= 向后跳过这些字符，并记录开始位置 =start= 。
  - 然后 =delete-region= 用于删除从 =start= 到当前光标位置之间的字符。
  - 如果光标前面不是连续空白字符，就调用 =puni-backward-delete-char= ，这是来自 =puni= 的函数，用于以结构化方式进行删除操作。

#+begin_src emacs-lisp :noweb-ref idiig/puni-backward-hungry-delete
...
#+begin_src emacs-lisp ::noweb-ref idiig/puni-backward-kill-word-or-region
