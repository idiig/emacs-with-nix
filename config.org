...
#+begin_src emacs-lisp
  ...
  (defun window-adjust-continue (orig-fun &rest args)
    ...
    (let ((echo-keystrokes 0)
      ...
      ;; 进入持续调整循环
      (while continue
	(let* ((key (read-event "Use ^, V, {, } for further adjustment)"))
	       (delta (car args)))
	  (cond
	   ;; 垂直调整
	   ((eq key ?^)
  ...
  ;; 如果需要移除 advice:
  ;; (advice-remove 'enlarge-window #'window-adjust-continue)
  ;; (advice-remove 'shrink-window #'window-adjust-continue)
  ;; (advice-remove 'enlarge-window-horizontally #'window-adjust-continue)
  ;; (advice-remove 'shrink-window-horizontally #'window-adjust-continue)

  ;; 为相关命令添加 advice
  (advice-add 'enlarge-window :around #'window-adjust-continue)
  (advice-add 'shrink-window :around #'window-adjust-continue)
  (advice-add 'enlarge-window-horizontally :around #'window-adjust-continue)
  (advice-add 'shrink-window-horizontally :around #'window-adjust-continue)
#+end_src
